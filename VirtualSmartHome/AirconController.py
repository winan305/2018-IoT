...
def on_connect(client, userdata, flags, rc) :
    # 에어컨 컨트롤러가 mqtt 서버에 연결되었음을 출력한다.
    print("Aricon Controller Connected.")

    # 에어컨 제어값을 subscribe 한다.
...
def controll(controll_msg) :
    # 메세지가 START 이면 에어컨을 start 함을 출력한다.
    if controll_msg == "START" :
        print("Start air conditioning")

    # 메세지가 STOP 이면 에어컨을 stop 함을 출력한다.
    elif controll_msg == "STOP" :
        print("Stop air conditioning")

# 호스트, 포트, keepalive를 전달받아 mqtt서버에 연결하고 루프를 시작하는 함수
